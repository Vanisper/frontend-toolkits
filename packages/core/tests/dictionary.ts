...
export function testDictionary() {
  ...
  AirAssert.when(
    dict1.length !== UserStatusDictionary.length,
    "Get dictionary from prototype error",
  );
  const dict2 = getDictionary(User, "status");
  AirAssert.when(
    dict2.length !== UserStatusDictionary.length,
    "Get dictionary from class error",
  );
  const dict3 = User.getDictionary("status");
  AirAssert.when(
    dict3.length !== UserStatusDictionary.length,
    "Get dictionary from static method error",
  );
